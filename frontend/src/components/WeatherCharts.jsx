      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
        <div>
          ...
          <h2 style={{ fontSize: "1.25rem", fontWeight: "700", color: "#ffffff", margin: "0.15rem 0" }}>
            7-Day Ensemble Forecast vs 30-Year Historical Climate
          </h2>
          <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
            Averaged from 3 Meteorological Sources (Open-Meteo, MET Norway, NOAA)
          </p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
        ...
        <div className="glass-panel" style={{ padding: "1rem" }}>
          <h3 style={{ fontSize: "0.9rem", color: "#ffffff", marginBottom: "0.75rem", fontWeight: "600" }}>
            Temperature (°C) Forecast vs 30-Year Baseline
          </h3>
          <div style={{ width: "100%", height: "220px" }}>
            <ResponsiveContainer>
              <ComposedChart data={chartData}>
                ...
                <Line type="monotone" dataKey="tempMax" name="Fcst Max Temp" stroke="#f43f5e" strokeWidth={2.5} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="tempMin" name="Fcst Min Temp" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="histTempMax" name="30-Yr Hist Max" stroke="#f59e0b" strokeDasharray="4 4" strokeWidth={1.5} dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: "1rem" }}>
          <h3 style={{ fontSize: "0.9rem", color: "#ffffff", marginBottom: "0.75rem", fontWeight: "600" }}>
            Water Balance (Precip vs PET) & VPD (kPa)
          </h3>
          <div style={{ width: "100%", height: "220px" }}>
            <ResponsiveContainer>
              <ComposedChart data={chartData}>
                ...
                <Tooltip contentStyle={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "8px", color: "#fff" }} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar yAxisId="left" dataKey="precipitation" name="Precip (mm)" fill="#06b6d4" radius={[4, 4, 0, 0]} />
                <Line yAxisId="left" type="monotone" dataKey="pet" name="PET Demand (mm)" stroke="#a855f7" strokeWidth={2} dot={false} />
                <Line yAxisId="right" type="monotone" dataKey="vpdMax" name="Max VPD (kPa)" stroke="#eab308" strokeWidth={2} dot={{ r: 3 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
