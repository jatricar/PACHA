...
export const translations = {
  es: {
    ...
    worldReport: {
      ...
      patienceNote: "La primera vez puede tardar bastante — a propósito va despacio (2 segundos entre cada consulta) para no saturar la API climática gratuita. Buena noticia: cada estación que se completa queda guardada para siempre, así que si esta corrida se corta a mitad de camino, la próxima retoma donde quedó en vez de arrancar de cero — y una vez que todas estén guardadas, generar el reporte es casi instantáneo. Podés navegar a otras secciones mientras tanto.",
      summaryLine: "Generado el {date} · {ok} de {total} estaciones procesadas correctamente ({cached} desde caché, {fresh} nuevas).",
  ...
  en: {
    ...
    worldReport: {
      ...
      patienceNote: "The first time can take a while - it deliberately runs slowly (a 2-second gap between requests) to avoid overloading the free weather archive API. Good news: every station that finishes is saved permanently, so if this run gets cut off partway, the next one picks up where it left off instead of starting over - and once every station is saved, generating the report is nearly instant. You can navigate to other sections meanwhile.",
      summaryLine: "Generated on {date} - {ok} of {total} stations processed successfully ({cached} from cache, {fresh} freshly fetched).",
